  ...
  return (
    <div className="s-card" role="button" tabIndex={0} onClick={goToDetail}
      ...
      <div style={{ padding: '.9rem 1rem 1rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: isEvent ? 'space-between' : 'flex-start', gap: '.3rem' }}>
          ...
          {isEvent && <span style={{ fontSize: '.78rem', fontWeight: 700, color: isGratis ? 'var(--jade)' : 'var(--carbon)', flexShrink: 0 }}>{price}</span>}
        </div>
      </div>
    </div>
  )
